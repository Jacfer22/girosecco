...
import { ProExtra } from '@/lib/types';
...
export default function ContenutoPro({ isPremium, proExtra, gpxUrl }: Props) {
  ...
  // Senza Supabase configurato non possiamo sapere se l'account è Pro:
  // mostriamo tutto, così l'anteprima resta utilizzabile.
  const sbloccato = nonConfigurato || !!profilo?.is_pro;

  if (isPremium && !sbloccato) {
    return (
      <>
        {/* Itinerario Pro, account free o non loggato: nessuna anteprima dei contenuti riservati */}
        <section className="mt-10 border-2 border-segnale bg-white">
          ...
          <div className="p-5">
            <h3 className="font-display text-2xl font-bold uppercase tracking-tight">
              Variante del percorso e pacchetto weekend
            </h3>
            <p className="mt-1 text-asfalto/85">
              Questo itinerario ha una variante del percorso e un pacchetto
              weekend completo (orari, soste, dove dormire), riservati agli
              iscritti Pro.
            </p>
          </div>
        </section>
        ...
        <section className="mt-10 border-2 border-asfalto bg-asfalto p-6 text-cemento">
          ...
          <p className="mt-2 text-guardrail">
            GPX, varianti del percorso e pacchetto weekend completo sono
            riservati agli iscritti Pro.
          </p>
          <Link
            href="/pro"
    ...
    <>
      ...
      <section className="mt-10 border-2 border-asfalto bg-asfalto p-6 text-cemento">
        ...
        {gpxUrl ? (
          <>
            <p className="mt-2 text-guardrail">
              Percorso proposto da GiroSecco: scaricala e caricala sul
              navigatore o sull’app che usi.
            </p>
            <a
              href={gpxUrl}
              download
